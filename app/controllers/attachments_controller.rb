class AttachmentsController < ApplicationController
  ...
  def create
    ...
    if @attachment.save
      redirect_to @attachment.product, notice: "Se guardo el archivo del producto"
    else
  ...
  def destroy
    @attachment.destroy
    redirect_to @product, notice: "Se ha elimidado el archivo"
  end
    ...
    def authenticate_owner!
        ...
        @product = Product.find(params[:attachment][:product_id])
  		...
  		if @product.nil? || @product.user != current_user
  			redirect_to root_path, notice: "No puedes editar el producto"
  			return
  		end
    end
